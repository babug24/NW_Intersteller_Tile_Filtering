  ...
  async generateReports(startTime, endTime, duration, tileStats = {}) {
    ...
    const jsonReport = {
      ...
      tileStats: {
        ...
        pagesWithTiles: tileStats.pagesWithTiles || 0,
...
Pages with Tiles: ${tileStats.pagesWithTiles || 0}/${this.results.length}
================================================================
RETRY STATISTICS:
  ...
  generateHTMLReport(jsonReport, timestamp) {
    ...
    jsonReport.results.forEach((result, index) => {
      ...
      combinations.slice(0, 10).forEach((combo, comboIndex) => {
        ...
        const statusClass = tileStatus === 'VALIDATED' ? 'success' : 
        ...
        <tr>
          ...
          <td>${tileInfo}</td>
          <td><span class="badge bg-${statusClass}">${tileStatus}</span></td>
        </tr>
        `;
      });
...
<body>
    <div class="header">
        <div class="container">
            <h1 class="display-4">📊 Dropdown Test Report with Tile Validation</h1>
