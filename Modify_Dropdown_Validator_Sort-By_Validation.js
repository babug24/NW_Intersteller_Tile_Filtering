...
class CompactDropdownTester {
  ...
  async validateTileCount(dropdownElements, currentSelection, comboNumber) {
    return this.executeWithRetry('tileValidation', async () => {
      ...
      return {
        ...
        status: tileData.visible > 0 ? 'VALIDATED' : 'NO_VISIBLE_TILES',
        validationTime: new Date().toISOString(),
        strategies: tileData.strategiesUsed,
        diagnostic: tileData.diagnostic
      };
  ...
  async testSingleCombination(selection, dropdownElements, comboNumber) {
    ...
    try {
      ...
      // 4. Set result status based on both tile and sort validation
      const hasVisibleTiles = tileValidation.visible > 0;
      const sortValidated = sortValidation.status === 'VALIDATED' || 
                           sortValidation.status === 'SKIPPED' && tileValidation.visible < 2;
      
      if (hasVisibleTiles && sortValidated) {
        result.status = 'PASSED';
        this.totalTests++; 
        this.passedTests++;
      } else {
        result.status = 'FAILED';
        result.error = !hasVisibleTiles ? 'No visible tiles found' : 'Sort validation failed';
        this.totalTests++; 
        this.failedTests++;
      }
  ...
  async generateReports(startTime, endTime, duration, tileSortStats = {}) {
    ...
    const jsonReport = {
      ...
      results: this.results.map(r => ({
        ...
        sortBySummary: r.sortBySummary || { validatedCombos: 0, skippedCombos: 0, combosWithSortControls: 0 },
    ...
    jsonReport.results.forEach((result, index) => {
      ...
      combinations.slice(0, 10).forEach((combo, comboIndex) => {
        ...
        const tileStatusClass = tileStatus === 'VALIDATED' ? 'success' : 
                               tileStatus === 'NO_VISIBLE_TILES' ? 'warning' : 'secondary';
        ...
        <tr>
          ...
          <td>${tileInfo}</td>
          <td><span class="badge bg-${tileStatusClass}">${tileStatus}</span></td>
...
<body>
    ...
    <div class="container">
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-3">
                <div class="card">
                    <div class="card-body text-center">
                        <h1 class="display-4 sort-count">${jsonReport.sortByStats.pagesWithSortControls}/${jsonReport.tileStats.totalPages}</h1>
                        <p class="text-muted mb-0">Pages with Sort Controls</p>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card">
                    <div class="card-body text-center">
                        <h1 class="display-4 sort-count">${jsonReport.sortByStats.averageSortControls}</h1>
                        <p class="text-muted mb-0">Avg Sort Controls</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <table class="table">
                            ...
                            <tr>
                                <td><strong>Pages with Controls:</strong></td>
                                <td>${jsonReport.sortByStats.pagesWithSortControls}/${jsonReport.tileStats.totalPages}</td>
                            </tr>
                            <tr>
    ...
    <script>
        ...
        const tileChart = new Chart(tileCtx, {
            type: 'bar',
            data: {
                labels: ['Total Tiles', 'Average per Page', 'Maximum', 'Minimum'],
                datasets: [{
                    label: 'Tile Statistics',
                    data: [${jsonReport.tileStats.totalTiles}, ${jsonReport.tileStats.averageTiles}, ${jsonReport.tileStats.maxTiles}, ${jsonReport.tileStats.minTiles}],
        ...
        const sortChart = new Chart(sortCtx, {
            type: 'pie',
            data: {
                labels: ['Validated', 'Skipped', 'Failed'],
                datasets: [{
                    data: [${jsonReport.sortByStats.validatedCombos}, ${jsonReport.sortByStats.skippedCombos}, ${jsonReport.sortByStats.validatedCombos ? 0 : jsonReport.sortByStats.skippedCombos ? 0 : 1}],
