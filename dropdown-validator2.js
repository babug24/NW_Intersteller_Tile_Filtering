...
https://www.nationwide.com/financial-professionals/topics/legacy-estate-wealth-transfer/,Nationwide Legacy & Estate Wealth Transfer,3`;
  ...
  async handleCookiesAndPopups() {
    try {
      console.log('🔍 Checking for cookies and popups...');
      ...
      try {
        ...
        if (await trusteAcceptButton.isDisplayed()) {
          console.log('🍪 Found Truste cookie banner');
          await trusteAcceptButton.click();
          console.log('✅ Clicked "Accept" on cookie banner');
          handled = true;
          ...
          try {
            ...
            for (const element of elements) {
              if (await element.isDisplayed()) {
                console.log('📍 Found California privacy popup');
                await element.click();
              ...
              if (await overlay.isDisplayed()) {
                ...
                for (const closeButton of closeButtons) {
                  if (await closeButton.isDisplayed()) {
                    ...
                    console.log('✅ Closed overlay/popup');
                    handled = true;
                    await this.driver.sleep(1500);
                    break;
                  }
      ...
      if (!handled) {
        try {
          ...
          if (await requiredOnlyButton.isDisplayed()) {
            console.log('📍 Found "Required only" button');
            await requiredOnlyButton.click();
            console.log('✅ Clicked "Required only"');
            handled = true;
            await this.driver.sleep(1500);
          }
        } catch (error) {
      ...
      if (handled) {
        console.log('✅ All cookies and popups handled');
      } else {
        console.log('ℹ️ No cookies or popups found');
      }
      
      return handled;
    } catch (error) {
      console.warn('⚠️ Error handling cookies/popups:', error.message);
      return false;
    }
  }
  ...
  async waitForPageLoad() {
    try {
      ...
      console.warn('⚠️ Page load warning:', error.message);
      return false;
    }
  }
  ...
  async getDropdownOptions(dropdownElement, index) {
    try {
      ...
      if (options.length === 0) {
        console.warn(`⚠️ No valid options found for dropdown ${index + 1}`);
        return [];
      }
  ...
  async resetToDefault(dropdownElements) {
    try {
      ...
      console.warn('⚠️ Reset warning:', error.message);
      return false;
    }
  }
      ...
      for (const [index, testCase] of this.csvData.entries()) {
        ...
        try {
          ...
          if (failed > 0) {
            ...
            console.log(`⚠️ URL Results: No combinations tested`);
          }
